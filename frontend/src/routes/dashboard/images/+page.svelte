<script lang="ts">
	...
	import { Image as ImageIcon, Download, Trash2, Users } from '@lucide/svelte';
	...
	async function pullImage() {
		if (!imageUrl || !$activeTeam?.team) return;
		pulling = true;
		const result = await api.images.pull(imageUrl, $activeTeam.team.id);
		if (result.data) {
			...
			{:else}
				<form onsubmit={(e) => { e.preventDefault(); pullImage(); }} class="flex gap-2">
					<Input
						...
						bind:value={imageUrl}
						class="flex-1"
						disabled={pulling || isViewer}
					/>
					<Button type="submit" disabled={pulling || !imageUrl || isViewer} class="gap-2">
